...
export const AssetManagementModule: React.FC<AssetManagementModuleProps> = ({
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'depreciation':
        return <Depreciation config={config} onDataRefresh={onDataRefresh} onNavigateToReports={() => setActiveTab('reports')} />;
      case 'disposal':
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      ...
      <div className="flex-1 overflow-hidden relative flex flex-col">
        {activeTab === 'reports' ? (
          <div className="absolute inset-0 flex flex-col min-h-0 overflow-hidden bg-slate-50">
            {renderContent()}
          </div>
        ) : (
