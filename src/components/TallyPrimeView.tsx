...
export const FinancialStatementView: React.FC<FinancialStatementViewProps> = ({
  ...
  const handleDepthSelect = (newDepth: ReportDetailDepth) => {
    setDepth(newDepth);
    if (onDepthChange) {
  ...
  const curSymbol = config?.CurrencySymbol || 'Nu.';
  ...
  const tbTree = useMemo(() => {
    ...
    const groupMap: Record<string, typeof filteredTb> = {};
    ...
    sortedGroupNames.forEach(grpName => {
      const ledgersInGrp = groupMap[grpName] || [];
  ...
  const pnlData = useMemo(() => {
    ...
    const turnover = s + di;
    const grossMarginPct = turnover > 0 ? ((grossProfit / turnover) * 100).toFixed(1) : '0.0';
    const netMarginPct = turnover > 0 ? ((netProfit / turnover) * 100).toFixed(1) : '0.0';
    ...
    return {
      ...
      turnover, grossMarginPct, netMarginPct
    };
  }, [reportData, depth]);
  ...
  const bsData = useMemo(() => {
    ...
    const workingCapital = (ca + stockVal) - cl;
  ...
  return (
    <div className="w-full font-sans text-xs sm:text-sm">
            ...
            <table className="w-full border-separate border-spacing-0 text-xs sm:text-sm">
              <thead className="sticky top-0 z-30 bg-slate-100 shadow-md ring-1 ring-slate-200">
                <tr className="bg-slate-100 text-slate-700 uppercase font-bold text-[11px] tracking-wider border-b border-slate-200">
                  ...
                  <th className="bg-slate-100 bg-clip-padding py-3 px-4 sm:px-6 text-right w-40 sm:w-52">Debit ({curSymbol})</th>
                  <th className="bg-slate-100 bg-clip-padding py-3 px-4 sm:px-6 text-right w-40 sm:w-52">Credit ({curSymbol})</th>
                </tr>
              </thead>
                        ...
                        >
                          <td className="py-3 px-4 sm:px-6">
                            <div className="flex items-center gap-2.5">
                              ...
                              <span className="text-slate-900 tracking-wide font-extrabold text-xs sm:text-sm">{groupNode.name}</span>
                              {groupNode.children && (
                          ...
                          <td className="py-3 px-4 text-xs font-semibold text-slate-500 hidden md:table-cell">
              ...
              <tfoot className="sticky bottom-0 z-30 bg-slate-100 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] ring-1 ring-slate-200">
                <tr className="bg-slate-100 border-t-2 border-slate-900 font-extrabold text-slate-900 text-xs sm:text-sm">
                  ...
                  <td className="bg-slate-100 bg-clip-padding py-3.5 px-4 hidden md:table-cell text-xs text-slate-500 font-normal">
                    {Math.abs(tbTotals.closingDr - tbTotals.closingCr) < 0.01 ? (
                      <span className="inline-flex items-center gap-1 text-emerald-700 font-bold">
                        <CheckCircle2 className="h-3.5 w-3.5" /> Perfectly Balanced
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 text-rose-600 font-bold">
                        <AlertTriangle className="h-3.5 w-3.5" /> Diff: {curSymbol} {fmt(Math.abs(tbTotals.closingDr - tbTotals.closingCr))}
                      </span>
                    )}
                  </td>
                  <td className="bg-slate-100 bg-clip-padding py-3.5 px-4 sm:px-6 text-right font-mono text-sm sm:text-base font-extrabold text-slate-900">
                    {curSymbol} {fmt(tbTotals.closingDr)}
                  </td>
                  <td className="bg-slate-100 bg-clip-padding py-3.5 px-4 sm:px-6 text-right font-mono text-sm sm:text-base font-extrabold text-slate-900">
                    {curSymbol} {fmt(tbTotals.closingCr)}
                  </td>
                </tr>
              </tfoot>
            </table>
          ...
          <div className="w-full bg-white">
                  ...
                  <div>
                    ...
                    <div className="space-y-3.5">
                      ...
                      {pnlData.grossProfit >= 0 && (
                        <div className="flex justify-between items-center font-extrabold text-emerald-900 bg-emerald-50 px-3 py-2 rounded-xl border border-emerald-200 mt-2">
                          <span className="flex items-center gap-1.5">
                            <TrendingUp className="h-4 w-4 text-emerald-600" />
                            Gross Profit c/o
                          </span>
                          <span className="font-mono text-sm sm:text-base font-extrabold text-emerald-800">{fmt(pnlData.grossProfit)}</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex flex-col justify-between p-4 sm:p-6 space-y-4">
                  <div>
                    ...
                    <div className="space-y-3.5">
                      ...
                      {pnlData.di > 0 && (
                        <div className="space-y-1">
                          <div className="flex justify-between items-center font-bold text-slate-900">
                            ...
                            <span className="font-mono text-slate-900">{fmt(pnlData.di)}</span>
                          </div>
                        </div>
                      )}
                      ...
                      {pnlData.grossProfit < 0 && (
                        <div className="flex justify-between items-center font-extrabold text-rose-900 bg-rose-50 px-3 py-2 rounded-xl border border-rose-200 mt-2">
                          <span className="flex items-center gap-1.5">
                            <TrendingDown className="h-4 w-4 text-rose-600" />
                            Gross Loss c/o
                          </span>
                          <span className="font-mono text-sm sm:text-base font-extrabold text-rose-800">{fmt(Math.abs(pnlData.grossProfit))}</span>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div>
              <div className="sticky top-0 z-30 bg-slate-100 px-4 sm:px-6 py-2 text-slate-800 font-extrabold text-xs uppercase tracking-wider border-b border-slate-200 flex items-center justify-between shadow-sm">
                ...
                <span className="text-[11px] text-slate-500 font-normal">Gross Margin +/- Indirect Incomes - Indirect Expenses</span>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-slate-200">
                ...
                <div className="flex flex-col justify-between p-4 sm:p-6 space-y-4">
                  <div className="space-y-3.5">
                    ...
                    {/* Nett Profit */}
                    {pnlData.netProfit >= 0 && (
                      <div className="flex justify-between items-center font-extrabold text-emerald-950 bg-emerald-50 px-3 py-2.5 rounded-xl border border-emerald-300 mt-3 shadow-xs">
                        <span className="flex items-center gap-1.5">
                          <CheckCircle2 className="h-4 w-4 text-emerald-600" />
                          Nett Profit (Transferred to Capital Account)
                        </span>
                        <span className="font-mono text-sm sm:text-base font-extrabold text-emerald-800">{fmt(pnlData.netProfit)}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="pt-3 border-t-2 border-slate-900 font-extrabold flex justify-between items-center text-slate-900 text-xs sm:text-sm">
                    ...
                    <span className="font-mono font-extrabold text-sm sm:text-base">
                      {curSymbol} {fmt(pnlData.ie + Math.max(0, -pnlData.grossProfit) + Math.max(0, pnlData.netProfit))}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-col justify-between p-4 sm:p-6 space-y-4">
                  <div className="space-y-3.5">
                    ...
                    {pnlData.grossProfit >= 0 && (
                      <div className="flex justify-between items-center font-bold text-slate-900">
                        ...
                        <span className="font-mono text-slate-900">{fmt(pnlData.grossProfit)}</span>
                      </div>
                    )}
                    ...
                    {/* Nett Loss if any */}
                    {pnlData.netProfit < 0 && (
                      <div className="flex justify-between items-center font-extrabold text-rose-950 bg-rose-50 px-3 py-2.5 rounded-xl border border-rose-300 mt-3 shadow-xs">
                        <span className="flex items-center gap-1.5">
                          <AlertTriangle className="h-4 w-4 text-rose-600" />
                          Nett Loss
                        </span>
                        <span className="font-mono text-sm sm:text-base font-extrabold text-rose-800">{fmt(Math.abs(pnlData.netProfit))}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="pt-3 border-t-2 border-slate-900 font-extrabold flex justify-between items-center text-slate-900 text-xs sm:text-sm">
                    <span className="uppercase tracking-wider">P&L Total</span>
                    <span className="font-mono font-extrabold text-sm sm:text-base">
                      {curSymbol} {fmt(Math.max(0, pnlData.grossProfit) + pnlData.ii + Math.max(0, -pnlData.netProfit))}
                    </span>
                  </div>
                </div>
              </div>
